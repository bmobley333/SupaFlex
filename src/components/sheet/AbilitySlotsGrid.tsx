...
export const AbilitySlotsGrid: React.FC<AbilitySlotsGridProps> = ({ title, type }) => {
  ...
  const handleEquipVaultItem = (vaultItem: MagicItem) => {
    ...
    updateActiveSheetData((prev) => {
      ...
      const newSlot: any = {
        ...
        source_mod: (vaultItem as any).source_mod,
        category: vaultItem.category || null,
        slot_weight: weight,
      };

      const inVault = currentVault.some((v) => cleanName(v.base_name || v.name).toLowerCase() === cleanTarget);
      const updatedVault = inVault ? currentVault : [...currentVault, vaultItem];
  ...
  const filteredRoster = useMemo(() => {
    ...
    return roster.filter((s) => {
      ...
      if (type === 'spells') {
        return matchFunctionSourceSearch(q, s, functionsCatalog, modsCatalog, activeCharacter);
      }
      return false;
    });
                                                  ...
                                                  >
                                                    <Star
                                                      className={`w-3.5 h-3.5 ${
                                                        isItemStarred(item) ? 'fill-amber-400 text-amber-400' : ''
                                                      }`}
                                                    />
                                                  </button>
                                                  <span className={`font-bold text-sm inline-flex items-center align-baseline ${isGsUnlocked && isMsoEntry(baseName) ? 'text-purple-300 font-bold' : 'text-slate-100'}`}>
