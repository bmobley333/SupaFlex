...
export const GmHeaderHUD: React.FC<GmHeaderHUDProps> = ({
  ...
  return (
    <div className="w-full pt-1.5 border-t border-slate-800/80 flex items-center justify-between flex-wrap gap-2 animate-fadeIn">
      {/* Left Zone: GM Screen Badge & Campaign Selector */}
      <div className="flex items-center gap-3">
        ...
        {parties.length > 0 && (
          <select
            ...
            onChange={(e) => {
              const selected = parties.find((p) => String(p.id) === e.target.value);
              if (selected) onSelectParty(selected);
            }}
            className="px-2.5 py-1 bg-slate-950/90 border border-slate-800 hover:border-amber-500/40 rounded-xl text-xs font-bold text-slate-200 outline-none cursor-pointer transition-all"
          >
            {parties.map((p) => (
              <option key={p.id} value={p.id}>
                {p.name} {p.room_code ? `(${p.room_code})` : ''}
              </option>
            ))}
          </select>
        )}
        ...
        {onOpenLaunchHub && (
          <button
            ...
            className="px-2.5 py-1 bg-slate-900 hover:bg-slate-800 text-slate-300 text-xs font-semibold rounded-lg border border-slate-800 transition cursor-pointer"
            title="Manage Campaigns & Parties in Vault"
          >
            Manage Parties
          </button>
        )}
