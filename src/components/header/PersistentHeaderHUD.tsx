...
import { ArrowDown, Zap, ChevronDown, ChevronUp, X, Plus, Minus } from 'lucide-react';
...
export const PersistentHeaderHUD: React.FC<PersistentHeaderHUDProps> = ({
  ...
  const [activeDrawer, setActiveDrawer] = useState<'none' | 'focus' | 'spark' | 'luck'>('none');
  ...
  const toggleDrawer = (target: 'focus' | 'spark' | 'luck') => {
    setActiveDrawer((prev) => (prev === target ? 'none' : target));
  };

  return (
    <div className="w-full flex items-center justify-center gap-3 flex-wrap" ref={containerRef}>
