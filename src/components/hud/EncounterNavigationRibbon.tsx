...
// High-Density Breadcrumb Navigation Ribbon with In-Dropdown Management, Tactical Notes & Push to Players
...
export const EncounterNavigationRibbon: React.FC<EncounterNavigationRibbonProps> = ({
  ...
  // Tactical Notes Drawer State
  const [isNotesOpen, setIsNotesOpen] = useState(false);
  const [tacticalNotesText, setTacticalNotesText] = useState('');
  ...
  // Sync tactical notes on encounter change
  useEffect(() => {
    setTacticalNotesText(activeEnc?.notes || activeEnc?.tactical_notes || '');
  }, [activeEnc?.id, activeEnc?.notes, activeEnc?.tactical_notes]);
  ...
  const handleNotesChange = (val: string) => {
    setTacticalNotesText(val);
    setEncounterNotes(val);
  };
  ...
  return (
    <div className={`flex flex-col gap-1.5 font-outfit ${className}`}>
      ...
      <div className="flex flex-wrap items-end justify-between gap-2.5 bg-slate-950/90 border border-slate-800 p-2.5 rounded-xl backdrop-blur-md shadow-md">
        ...
        <div className="flex items-center flex-wrap gap-2 flex-1 min-w-[320px]">
          {/* 1. ADVENTURE Dropdown */}
          <div className="flex flex-col gap-0.5 relative" ref={advMenuRef}>
            <span className="text-[9px] font-extrabold uppercase tracking-wider text-indigo-400 pl-1 font-mono">
              Adventure
            </span>
            <button
              type="button"
            ...
            >
              <div className="flex items-center gap-1.5 truncate">
                ...
                <span className="truncate">{activeAdv?.title || 'Select Adventure...'}</span>
              </div>
          ...
          <div className="flex flex-col gap-0.5 relative" ref={actMenuRef}>
            <span className="text-[9px] font-extrabold uppercase tracking-wider text-amber-400 pl-1 font-mono">
              Act / Chapter
            </span>
            <button
              type="button"
            ...
            >
              <div className="flex items-center gap-1.5 truncate">
                ...
                <span className="truncate">{activeAct?.title || 'Select Act...'}</span>
              </div>
          ...
          <div className="flex flex-col gap-0.5 relative" ref={encMenuRef}>
            <span className="text-[9px] font-extrabold uppercase tracking-wider text-rose-400 pl-1 font-mono">
              Encounter
            </span>
            <button
              type="button"
            ...
            >
              <div className="flex items-center gap-1.5 truncate">
                ...
                <span className="truncate">{activeEnc?.title || 'Select Encounter...'}</span>
              </div>
        ...
        {/* Right: Room Stepping & Action Buttons */}
        <div className="flex items-center gap-1.5 shrink-0">
          {/* Stepping Controls with Auto-Act Traversal */}
          <div className="flex items-center bg-slate-900/90 border border-slate-800 rounded-lg p-0.5 shadow-inner">
            <button
              ...
              className="px-2 py-1 text-slate-300 hover:text-white hover:bg-slate-800 rounded transition cursor-pointer text-xs font-bold flex items-center gap-1"
              title="Step Back to Previous Encounter (traverses across Acts)"
            >
            ...
            <button
              ...
              className="px-2 py-1 text-slate-300 hover:text-white hover:bg-slate-800 rounded transition cursor-pointer text-xs font-bold flex items-center gap-1"
              title="Step Forward to Next Encounter (traverses across Acts)"
            >
              <span>Next</span>
      ...
      {/* Collapsible 1-Line Tactical Notes Drawer */}
      {isNotesOpen && (
        <div className="bg-slate-950/95 border border-amber-500/40 p-2 rounded-xl flex flex-col gap-1 shadow-inner animate-in fade-in duration-150">
          <div className="flex items-center justify-between text-[10px] font-bold text-amber-400 uppercase tracking-wider">
            <span className="flex items-center gap-1">
              <StickyNote className="w-3 h-3" /> Tactical Room Notes & Traps ({activeEnc?.title || 'Encounter'})
            </span>
            <button
              type="button"
          ...
          <textarea
            rows={2}
            value={tacticalNotesText}
            onChange={(e) => handleNotesChange(e.target.value)}
            placeholder="e.g. Floor spikes trigger on round 2; 2 skeleton archers on upper catwalk; secret door behind altar..."
            className="w-full bg-slate-900 border border-slate-800 rounded-lg p-2 text-xs text-slate-100 font-mono outline-none focus:border-amber-500"
          />
        </div>
      )}
    </div>
